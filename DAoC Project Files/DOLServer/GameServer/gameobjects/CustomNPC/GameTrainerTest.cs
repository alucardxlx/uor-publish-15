...
namespace DOL.GS
{
	...
	/// </summary>
	public class GameTrainerTest : GameNPC
	{
		...
		{
			//Oblivion: Not used, remove later
			Acolyte = 4,
			AlbionRogue = 2,
			Disciple = 7,
			Elementalist = 5,
		...
		/// </summary>
		public GameTrainerTest()
		{
		}
		/// <summary>
		/// Constructs a new GameTrainer that will also train Champion levels
		/// </summary>
		public GameTrainerTest(eChampionTrainerType championTrainerType)
		{
		...
		{
			return true; //Oblivion: This is our advanced spec line trainer, needs to be true...should remove this check altogether.
		}
		...
		public static void RemoveLine(GamePlayer player, string line, bool answer)
		{
			if (answer != true) return; //Respec/remove declined
			int specPoints = 0;
			Specialization specLine = player.GetSpecialization(line);
			...
			if (specPoints > 0)
			{
				...
				player.RemoveAllStyles();
				player.UpdateSpellLineLevels(false);
				player.Out.SendMessage("You regain " + specPoints + " specialization points and have been removed from the path of " + line + "!", eChatType.CT_Say, eChatLoc.CL_PopupWindow);
			}
			if (player.GetSpellLine(line) != null) //Oblivion: Armor and other lines are not spell lines
			{
				player.RemoveSpellLine(line);
			}
			player.RemoveSpecialization(line);
			player.RefreshSpecDependantSkills(false);
			player.Out.SendUpdatePlayerSkills();
			player.Out.SendUpdatePoints();
			player.Out.SendUpdatePlayer();
