        ...
        protected virtual bool PvE_IsAllowedToAttack(GameLiving attacker, GameLiving defender, bool quiet)
        {
            ...
            if (attacker.Realm != 0 && defender.Realm != 0)
            {
                if (attacker is GamePlayer && ((GamePlayer)attacker).DuelTarget != defender)
                {
                    ...
                    if (quiet == false) MessageToLiving(attacker, "You can not attack other players in this zone.");
                    return false;
                }
            }
            ...
            if (attacker.Realm == 0 && defender.Realm == 0)
                return false;
            // Mobs can not attack mobs with realm assignment in PVE zones. ( Vendors attacking vendors)
            if (attacker is GameNPC && defender is GameNPC && (attacker.Realm == defender.Realm))
                return false;
                            ...
                            lock (killerPlayer.Group)
                            {
                                ...
                                foreach (GamePlayer player in killerPlayer.Group.GetPlayersInTheGroup())
                                {
                                   // if (!WorldMgr.CheckDistance(player, killedPlayer, WorldMgr.MAX_EXPFORKILL_DISTANCE)) continue;
                                    if(living.IsWithinRadius(player, WorldMgr.MAX_EXPFORKILL_DISTANCE)) continue;
                                    count++;
                                }
